"use client";

import { motion } from "framer-motion";
import SpinnerSmall from "./SpinnerSmall";
import { useLang } from "./LanguageProvider";

export default function ScaleCard({
  scale,
  index,
  onViewData,
  perScaleSyncing,
  onViewCharts,
}) {
  const { t } = useLang();

  return (
    <motion.div
      key={scale.scale_id}
      className="min-w-[280px] snap-start border-b-1 border-l-1 rounded-2xl p-4 
                 hover:shadow-lg shadow-amber-200 transition"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: index * 0.1, duration: 0.5 }}
    >
      {/* Scale basic info */}
      <h2 className="text-xl font-semibold text-gray-500">
        <span className="text-gray-500 text-sm">{t("scaleCard.name")}: </span>
        {scale.name}
      </h2>

      <p className="text-sm text-gray-500">
        {t("scaleCard.serial")}: {scale.serial_number}
      </p>

      <p className="text-sm text-gray-500">
        {t("scaleCard.scaleId")}: {scale.scale_id}
      </p>

      <p className="text-sm text-gray-500">
        {t("scaleCard.hwKey")}: {scale.hardware_key}
      </p>

      {/* "View Data" + spinner */}
      <div className="flex items-center gap-2 mt-4">
        <button
          onClick={() => onViewData(scale.scale_id)}
          className="bg-green-600 text-white px-4 py-2 rounded"
        >
          📊 {t("scaleCard.viewData")}
        </button>

        {perScaleSyncing[scale.scale_id] && (
          <SpinnerSmall
            mt="mt-0"
            mx="mx-auto"
            w="w-8"
            h="h-8"
            border="border-green-600"
          />
        )}
      </div>

      {/* "View Charts" */}
      <button
        onClick={() => onViewCharts(scale.scale_id)}
        className="mt-2 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-500 transition"
      >
        📈 {t("scaleCard.viewCharts")}
      </button>
    </motion.div>
  );
}
